...
export interface PulseraCompleta extends Omit<
  Pulsera,
...
> {
  ...
  variaciones_color?: { color_nombre: string; hex: string }[]; // Ahora es opcional
}
...
export function resolverPulsera(p: Pulsera): PulseraCompleta {
  return {
    ...
    variaciones_color: p.color_ids
      ...
      : undefined, // Devuelve undefined si color_ids no existe
  };
}
...
export function getPulseraPorSlug(slug: string): PulseraCompleta | undefined {
  const p = PULSERAS_DATA.find(
    (p) => p.id.toLowerCase() === slug.toLowerCase(),
  );
  return p ? resolverPulsera(p) : undefined;
}

export function getPulserasPorCategoria(
  categoriaId: string,
): PulseraCompleta[] {
  return PULSERAS_DATA.filter((p) => p.categoria_ids.includes(categoriaId)).map(
    resolverPulsera,
  );
}
